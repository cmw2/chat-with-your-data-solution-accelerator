...
// output deployAdminWeb bool = deployAdminWeb
// output deployFunction bool = deployFunction
// output deployOpenAI bool = deployOpenAI
// output deploySearch bool = deploySearch
// output azureOpenAIResourceName string = azureOpenAIResourceName
// output azureAISearchName string = azureAISearchName
// output existingAOAIResourceGroup string = existingAOAIResourceGroup
// output existingSearchResourceGroup string = existingSearchResourceGroup
...
resource rgExistingAOAI 'Microsoft.Resources/resourceGroups@2021-04-01' existing = {
  name: existingAOAIResourceGroup
}

resource rgExistingSearch 'Microsoft.Resources/resourceGroups@2021-04-01' existing = {
  name: existingSearchResourceGroup
}
...
resource openai_existing 'Microsoft.CognitiveServices/accounts@2023-05-01' existing = {
  scope: rgExistingAOAI
  name: azureOpenAIResourceName
}

// module computerVision 'core/ai/cognitiveservices.bicep' = if (deployOpenAI && useAdvancedImageProcessing) {
//   name: 'computerVision'
//   scope: rg
//   params: {
...
// module searchIndexRoleOpenai 'core/security/role.bicep' = if (deployOpenAI && authType == 'rbac') {
//   scope: rg
...
// module searchServiceRoleOpenai 'core/security/role.bicep' = if (deployOpenAI && authType == 'rbac') {
//   scope: rg
...
// module blobDataReaderRoleSearch 'core/security/role.bicep' = if (deploySearch && authType == 'rbac') {
//   scope: rg
...
// module openAiRoleSearchService 'core/security/role.bicep' = if (deploySearch && authType == 'rbac') {
//   scope: rg
...
// module search './core/search/search-services.bicep' = if (deploySearch) {
...
resource search_existing 'Microsoft.Search/searchServices@2021-04-01-preview' existing = {
  scope: rgExistingSearch
  name: azureAISearchName
}
...
module web './app/web.bicep' = if (hostingModel == 'code') {
  ...
  params: {
    ...
    azureOpenAIName: openai_existing.name
    azureAISearchName: search_existing.name
    storageAccountName: storage.outputs.name
    formRecognizerName: formrecognizer.outputs.name
    contentSafetyName: contentsafety.outputs.name
...
// module web_docker './app/web.bicep' = if (deployWeb && hostingModel == 'container') {
...
//     applicationInsightsName: monitoring.outputs.applicationInsightsName
//     healthCheckPath: '/api/health'
//     azureOpenAIName: deployOpenAI ? openai.outputs.name : openai_existing.name
//     azureAISearchName: deploySearch ? search.outputs.name : search_existing.name
//     storageAccountName: storage.outputs.name
//     formRecognizerName: formrecognizer.outputs.name
//     contentSafetyName: contentsafety.outputs.name
//     speechServiceName: speechService.outputs.name
...
// module adminweb './app/adminweb.bicep' = if (deployAdminWeb && hostingModel == 'code') {
...
// module adminweb_docker './app/adminweb.bicep' = if (deployAdminWeb && hostingModel == 'container') {
...
// module workbook './app/workbook.bicep' = if (deployWeb || deployAdminWeb || deployFunction){
//   name: 'workbook'
//   scope: rg
//   params: {
...
//     functionName: deployFunction ? (hostingModel == 'container' ? function_docker.outputs.functionName : function.outputs.functionName) : ''
//     websiteName: deployWeb ? (hostingModel == 'container' ? web_docker.outputs.FRONTEND_API_NAME : web.outputs.FRONTEND_API_NAME) : ''
//     adminWebsiteName: deployAdminWeb ? (hostingModel == 'container'
...
// module function './app/function.bicep' = if (deployFunction && hostingModel == 'code') {
//   name: functionName
//   scope: rg
//   params: {
//     name: functionName
...
// module function_docker './app/function.bicep' = if (deployFunction && hostingModel == 'container') {
...
//     azureOpenAIName: deployOpenAI ? openai.outputs.name : openai_existing.name
//     azureAISearchName: deploySearch ? search.outputs.name : search_existing.name
//     azureAISearchEndpoint: deploySearch ? search.outputs.endpoint : 'https://${azureAISearchName}.search.windows.net/'
//     azureOpenAIEndpoint: deployOpenAI ? openai.outputs.name : openai_existing.properties.endpoint
//   }
// }
...
output AZURE_BLOB_ACCOUNT_KEY string = ''
output AZURE_COMPUTER_VISION_ENDPOINT string = ''
output AZURE_COMPUTER_VISION_KEY string = ''
...
output AZURE_CONTENT_SAFETY_KEY string = ''
output AZURE_FORM_RECOGNIZER_ENDPOINT string = formrecognizer.outputs.endpoint
output AZURE_FORM_RECOGNIZER_KEY string = ''
output AZURE_KEY_VAULT_ENDPOINT string = ''
output AZURE_KEY_VAULT_NAME string = ''
...
output AZURE_SEARCH_SERVICE string = 'https://${azureAISearchName}.search.windows.net/'
...
output AZURE_SPEECH_SERVICE_KEY string = ''
...
output FRONTEND_WEBSITE_NAME string = hostingModel == 'code' ? web.outputs.FRONTEND_API_URI : ''
output ADMIN_WEBSITE_NAME string = ''
...
output AZURE_ML_WORKSPACE_NAME string = ''
